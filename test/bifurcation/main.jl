...
#
# <a name="grafo"></a>
grafo = LightGraphs.Graph(length(model[:,1])+1)
...
# `edgess` is a list of all the edges in `grafo`
edgess = LightGraphs.edges(grafo)
...
if (isfile(v_model)) == true
  ...
  grafo_v =LightGraphs.simple_graph(length(model_v[:,1])+1)
 ...
 LightGraphs.add_edge!(grafo_v, vessels_v[1].sn, vessels_v[1].tn)
  ...
  edgess_v =LightGraphs.edges(grafo_v)

  dts_v  = zeros(Float64, length(edgess_v))
...
prog = ProgressMeter.Progress(convert(Int, ceil(total_time/dt)), 1, "Running ", 50)
...
while true
  ...
  #Solve veins
  if venous_model
    openBF.solveModel(grafo_v, edgess_v, vessels_v,
