...
fldr_idx = 0
pext = ["Pext = 0", "Pext = g"]
for fldr in ["no-p_ext/modelathon_results", "modelathon_results"]:
	...
	for a in arteries:
		...
		print np.sum(qA[:,3])*1e6*(qA[1,0]-qA[0,0])

		ax1.plot(pA[:,0], pA[:,3]/133.332, label=("%s %s" %(a, pext[fldr_idx])))
