...
#           @printf "node %i -> anastomosis(%i, %i, %i) \n" t Graphs.edge_index(es[1],g) Graphs.edge_index(es[2], g) Graphs.edge_index(Graphs.out_edges(t,g)[1])
#           openBF.joinVessels(blood, vessels[i], 1, vessels[Graphs.edge_index(o[1], grafo)])
...
function solveModel(grafo, edgess, vessels,
                    grafo_a, edgess_a, vessels_a,
                    blood :: Blood, dt :: Float64, current_time :: Float64)

  for i in 1:length(edgess)
    s = Graphs.src(edgess[i])
    t = Graphs.dst(edgess[i])

    if Graphs.indegree(grafo)[s] == 0
      rn = vessels[i].rn
      edg = Graphs.inneighbors(rn, grafo_a)
      src = Graphs.edge_index(edg[1], grafo_a)
      artery = vessels_a[src]
      openBF.setInletBC(vessels[i], dt, artery.Pc, artery.R2)
    end
    ...
    elseif Graphs.outdegree(grafo)[t] == 1
      if length(inneighbors(grafo, t)) == 1
        o = Graphs.outneighbors(grafo, t)
        openBF.joinVessels(blood, vessels[i], vessels[Graphs.edge_index(o[1], grafo)])

      else
        es = Graphs.inneighbors(grafo, t)
        if i == max(Graphs.edge_index(es[1], grafo), Graphs.edge_index(es[2], grafo))
          a = Graphs.edge_index(es[1], grafo)
          b = Graphs.edge_index(es[2], grafo)
          c = Graphs.edge_index(Graphs.outneighbors(grafo, t)[1])
          openBF.solveAnastomosis(vessels[a], vessels[b], vessels[c])
        end
      end

    elseif Graphs.outdegree(grafo)[t] == 2
      openBF.joinVessels(blood, vessels[i], vessels[Graphs.edge_index(Graphs.outneighbors(grafo, t)[1])],
                        vessels[Graphs.edge_index(Graphs.outneighbors(grafo, t)[2])])
    end
  end
end

function pSolveModel(grafo, edgess, vessels, heart :: Heart,
                    blood :: Blood, dt :: Float64, current_time :: Float64)
